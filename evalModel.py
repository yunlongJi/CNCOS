...
def train_and_evaluate(input_data, config, random_state=0):
    ...
    att_sim_s = input_data['att_sim_s']
    att_sim_t = input_data['att_sim_t']
    ...
    n_input = X_s.shape[1]
    num_class_s = Y_s.shape[1]  # 1：列：即标签的类别数量
    num_nodes_S = X_s.shape[0]  # 0：行：即源域中节点的数量
    num_nodes_T = X_t.shape[0]  # same
